...
public class BackCrossSuffixRuleTest {
	...
	public static final String TEST_BASE_CROSS_NAME = "Cross";
    public static final int MALE_GID = 100;
    public static final int FEMALE_GID = 101;
	...
	public void testRunRuleMaleRecurrent() throws RuleException, MiddlewareQueryException {
		Mockito.when(pedigreeDataManager.calculateRecurrentParent(Mockito.anyInt(), Mockito.anyInt())).thenReturn(
                PedigreeDataManager.MALE_RECURRENT);

		String output = (String) unitUnderTest.runRule(ruleContext);
		Assert.assertEquals("Unable to add proper suffix for items with recurrent male parent", TEST_BASE_CROSS_NAME + BackCrossSuffixRule.MALE_RECURRENT_SUFFIX, output);
	}

	@Test
	public void testRunRuleFemaleRecurrent() throws RuleException, MiddlewareQueryException  {
		Mockito.when(pedigreeDataManager.calculateRecurrentParent(Mockito.anyInt(), Mockito.anyInt())).thenReturn(
				PedigreeDataManager.FEMALE_RECURRENT);

		String output = (String) unitUnderTest.runRule(ruleContext);
		Assert.assertEquals("Unable to add proper suffix for items with recurrent female parent", TEST_BASE_CROSS_NAME + BackCrossSuffixRule.FEMALE_RECURRENT_SUFFIX, output);
	}

	@Test
	public void testRunRuleNonRecurrent() throws RuleException, MiddlewareQueryException  {
		Mockito.when(pedigreeDataManager.calculateRecurrentParent(Mockito.anyInt(), Mockito.anyInt())).thenReturn(PedigreeDataManager.NONE);
