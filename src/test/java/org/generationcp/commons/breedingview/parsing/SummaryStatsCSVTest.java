...
public class SummaryStatsCSVTest {
	...
	public void testGetHeaders() throws IOException {
		...
		allHeaders.add(TRIAL_INSTANCE);
		allHeaders.add("TRAIT");
		allHeaders.addAll(SUMMARY_STATS_LIST);
	...
	public void testGetSummaryHeaders() throws IOException {
		...
		Assert.assertEquals(SUMMARY_STATS_LIST.size(), summaryHeaders.size());
		final ListIterator<String> expectedHeadersIterator = SUMMARY_STATS_LIST.listIterator();
	...
	public void testGetTrialHeader() throws IOException {
		Assert.assertEquals(TRIAL_INSTANCE, this.summaryStatsCSV.getTrialHeader());
	}
	
	@Test
	public void testGetData() throws IOException {
		final String environmentName = "1";
		final Map<String, Map<String, ArrayList<String>>> summaryData = this.summaryStatsCSV.getData();
		...
		Assert.assertEquals(TRAITS_LIST.size(), summaryData.get(environmentName).size());
		for (final String trait : TRAITS_LIST) {
		...
		final ArrayList<String> valuesForTrait1 = summaryData.get(environmentName).get(TRAITS_LIST.get(0));
		...
		final ArrayList<String> valuesForTrait2 = summaryData.get(environmentName).get(TRAITS_LIST.get(1));
		...
		final ArrayList<String> valuesForTrait3 = summaryData.get(environmentName).get(TRAITS_LIST.get(2));
		...
		final ArrayList<String> valuesForTrait4 = summaryData.get(environmentName).get(TRAITS_LIST.get(3));
