...
public class DateUtil {
	...
	public static boolean isValidDate(int year, int month, int day) {
		boolean yearOk = DateUtil.isValidYear(year);
		boolean monthOk = month >= 1 && month <= 12;
		boolean dayOk = day >= 1 && day <= DateUtil.daysInMonth(year, month);
	...
	public static Integer getYear(Date date) {
		String year = DateUtil.getSimpleDateFormat("yyyy").format(date);
		return Integer.parseInt(year);
	}

	/**
	 * Returns integer value of year month date concatenation if no date value, yyyymm eg. if month = 1, and year = 2005 => 200501 if no
	 * month and date, yyyy
	 * 
	...
	public static boolean isValidYear(Date date) {
		return DateUtil.isValidYear(DateUtil.getYear(date));
	}
	...
	public static String convertDate(String date, String oldFormat, String newFormat) throws ParseException {
		SimpleDateFormat sdf = DateUtil.getSimpleDateFormat(oldFormat);
	...
	public static String getCurrentDateInUIFormat() {
		return Util.getCurrentDateAsStringValue(Util.FRONTEND_DATE_FORMAT);
	}
	...
	public static String getDateInUIFormat(Date date) {
		...
		return Util.formatDateAsStringValue(date, Util.FRONTEND_DATE_FORMAT);
	}
	...
	public static String convertToUIDateFormat(Integer dataTypeId, String value) {
		String returnVal = value;
		if (dataTypeId != null && dataTypeId == TermId.DATE_VARIABLE.getId() && value != null && !"".equalsIgnoreCase(value)) {
			try {
				return DateUtil.convertDate(value, DateUtil.DATE_AS_NUMBER_FORMAT, DateUtil.FRONTEND_DATE_FORMAT);
			} catch (ParseException e) {
				DateUtil.LOG.error(e.getMessage(), e);
				returnVal = "";
			}
	...
	public static boolean isValidDate(String dateString) {
		if (dateString == null || dateString.length() != DateUtil.DATE_AS_NUMBER_FORMAT.length()) {
			return false;
		}
		...
		return DateUtil.isValidDate(year, month, day);
	}
