...
public class MySQLUtil {
	...
	public boolean upgradeDatabase(final Connection connection, final String databaseName, final File updateDir) throws Exception {
		...
		try {
			final String disableFk = "SET FOREIGN_KEY_CHECKS=0";
			...
			final List<File> schemaUpdateList = Arrays.asList(updateDir.listFiles(new FileFilter() {
			...
			for (final File schemaUpdateDir : schemaUpdateList) {
				if (currentSchemaVersion != null) {
					final int compareResult = schemaUpdateDir.getName().compareTo(currentSchemaVersion);
					if (compareResult <= 0) {
						continue;
					}
				}
			...
			final String enableFk = "SET FOREIGN_KEY_CHECKS=1";
